...
import { CreateDoctorData, UpdateDoctorData, DoctorStatus } from '@/types/doctor.type';
...
export function DoctorForm({ doctor, onSuccess }: DoctorFormProps) {
  ...
  const [profileImageId, setProfileImageId] = useState<string>(doctor?.user?.profile_image_id || '');
  ...
  const mutation = useMutation({
    ...
    onError: (error: any) => {
      const errorMessage = error?.response?.data?.message || (isEditMode ? 'Failed to update doctor' : 'Failed to create doctor');
    ...
    <form onSubmit={handleSubmit(handleFormSubmit)} className="space-y-4" noValidate>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
          ...
          <div className="md:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-x-3 gap-y-10 md:place-content-center-safe">
            <div>
              ...
              <input
                type="text"
                {...register('user.name', { required: 'Full name is required' })}
              ...
              <input
                ...
                {...register('user.email', {
                  ...
                  pattern: {
                    ...
                    message: 'Email must include @ and a valid domain (e.g., user@example.com)',
                  },
                })}
            ...
            <div>
              ...
              <input
                ...
                className={`w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 ${
                  errors.user?.phone_number ? 'border-red-500' : 'border-gray-300'
                }`}
      ...
      <div className="flex justify-end space-x-4 pt-6 border-t">
        ...
        <Button type="submit" disabled={mutation.isPending}>
          {mutation.isPending ? 'Saving...' : isEditMode ? 'Update Doctor' : 'Create Doctor'}
        </Button>
      </div>
